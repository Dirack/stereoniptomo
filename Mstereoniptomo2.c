...
int main(int argc, char* argv[])
{
	...
	int nsv[2]; // Dimension of sv vector
	float osv[2];
	float dsv[2];
	...
	sf_floatread(sv,nsv[0]*nsv[1],vz_file);

	/* VFSA parameters vectors */
	cnewv = sf_floatalloc(nsv[0]*nsv[1]);
	otsv = sf_floatalloc(nsv[0]*nsv[1]);
	...
	buildSlownessModelFromVelocityModel2(slow,n,o,d,sv,nsv,osv,dsv);
	...
	sf_putint(vspline,"n1",nsv[0]);
	sf_putint(vspline,"n2",nsv[1]);
	...
	for (q=0; q<nit; q++){
		...
		disturbParameters2(temp,cnewv,sv,nsv,1.46,1.9,1);

	//	dumpfloat1("cnewv",cnewv,nsv[0]*nsv[1]);

		/* Function to update velocity model */
		buildSlownessModelFromVelocityModel2(slow,n,o,d,cnewv,nsv,osv,dsv);
	...
	updateVelocityModel2(slow,n,o,d,otsv,nsv,osv,dsv);

	/* Write velocity cubic spline function */
	sf_floatwrite(otsv,nsv[1]*nsv[0],vspline);
